...
export default function SellerOrdersPage() {
  ...
  // Get seller orders directly (no store dependency)
  const {
    data: ordersData,
    ...
    isFetchingNextPage,
    refetch,
  } = useInfiniteSellerOrders(20)
  const confirmOrderMutation = useConfirmOrder()
  const updateOrderStatusMutation = useUpdateOrderStatus()

  // Handle new order notifications - auto refresh order list
  const handleNewOrderNotification = useCallback(
    (notification: NotificationPayload) => {
      // Only refresh for order-related notifications
      if (notification.type === 'order') {
        refetch()
        // Switch to pending tab to show new order
  ...
  useNotificationSocket({
    onNotification: handleNewOrderNotification,
    showToast: true, // Show toast for new orders
  })
                      ...
                      <CardContent className="flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-muted-foreground text-center max-w-md">
                          {status === 'all'
                            ...
                            : `Chưa có đơn hàng ${STATUS_CONFIG[status as keyof typeof STATUS_CONFIG]?.label.toLowerCase() || ''}`}
                        </p>
                      </CardContent>
                    </Card>
                  ) : (
