...
export function useUpdateOrderPaymentStatus() {
  ...
  return useMutation({
    ...
    }) => adminApi.updateOrderPaymentStatus(id, paymentStatus),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-orders'] })
      queryClient.invalidateQueries({ queryKey: ['admin-order-stats'] })
      queryClient.invalidateQueries({ queryKey: ['admin-dashboard-stats'] })
...
export function useBulkApproveProducts() {
  ...
  return useMutation({
    mutationFn: (productIds: string[]) =>
      adminApi.bulkApproveProducts(productIds),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['moderation-products'] })
      queryClient.invalidateQueries({ queryKey: ['moderation-stats'] })
    },
...
export function useBulkRejectProducts() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      productIds: string[]
      reason?: string
    }) => adminApi.bulkRejectProducts(productIds, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['moderation-products'] })
      queryClient.invalidateQueries({ queryKey: ['moderation-stats'] })
    },
...
export function useAssignTicket() {
  ...
  return useMutation({
    mutationFn: ({ id, assignedTo }: { id: string; assignedTo: string }) =>
      adminApi.assignTicket(id, assignedTo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-tickets'] })
    },
  })
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['admin-ticket', variables.id],
      })
      queryClient.invalidateQueries({ queryKey: ['admin-tickets'] })
    },
  })
...
export function useUpdateSystemSettings() {
  ...
  return useMutation({
    mutationFn: (
      settings: Parameters<typeof adminApi.updateSystemSettings>[0]
...
export function useUpdateSettingsByCategory<T>() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['admin-system-settings'] })
      queryClient.invalidateQueries({
        queryKey: ['admin-settings-category', variables.category],
      })
    },
  })
}
...
export function useCreateDepositPackage() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data: CreateDepositPackageData) =>
      adminApi.createDepositPackage(data),
    onSuccess: () => {
