...
export function DemoDataLoader({ onDataLoaded }: DemoDataLoaderProps) {
  ...
  const parseCSV = useCallback((csvText: string): ParsedData => {
    ...
    const products: Product[] = data.map((row, index) => ({
      ...
      images: row.images ? (row.images as string).split(',').map((img: string) => img.trim()) : ['https://images.unsplash.com/photo-1592899677977-9c10b588e3e9?w=400&h=400&fit=crop'],
    ...
    if (Array.isArray(data)) {
      return {
        products: data.map((item: Record<string, unknown>, index) => ({
          ...
          images: (item.images as string[]) || ['https://images.unsplash.com/photo-1592899677977-9c10b588e3e9?w=400&h=400&fit=crop'],
