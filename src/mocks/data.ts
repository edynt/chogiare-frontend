...
function generateModerationProducts(): ModerationProduct[] {
  const statuses: Array<'pending' | 'approved' | 'rejected' | 'draft'> = [
    'pending',
    'approved',
    'rejected',
    'draft',
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    products.push({
      ...
      reviewedAt:
        status !== 'pending'
          ? new Date(
              submittedDate.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000
            ).toISOString()
          : undefined,
...
export const demoData: DemoData = {
  ...
  orders: sampleData.orders.map((order, index) => {
    ...
    const userEmails = [
      ...
      'buithih@example.com',
      'ngovani@example.com',
      'duongthik@example.com',
      'lyvanl@example.com',
      'vothim@example.com',
      'dovann@example.com',
      'phanthio@example.com',
      'truongvanp@example.com',
    ]
    const addresses = [
    ...
    return {
      ...
      total:
        order.total - (index % 3 === 0 ? Math.floor(order.total * 0.1) : 0),
      currency: 'VND',
