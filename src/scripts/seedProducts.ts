...
function generateUsers(count: number): User[] {
  return Array.from({ length: count }, (_, index) => ({
    ...
    sellerLogo: faker.image.url(),
    sellerBanner: faker.image.url(),
    sellerAddress: faker.location.streetAddress(),
    sellerPhone: faker.phone.number(),
    sellerEmail: faker.internet.email(),
    isSellerVerified: faker.datatype.boolean({ probability: 0.7 }),
    sellerRating: faker.number.float({ min: 3.5, max: 5, fractionDigits: 1 }),
    sellerReviewCount: faker.number.int({ min: 10, max: 500 }),
